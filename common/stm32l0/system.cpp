...
static void SetSysClock (void) {
  ...
  //Flash.ACR.R = FLASH_ACR_PRFTBE | FLASH_ACR_LATENCY;
  Flash.ACR.setbits([] (auto & r) -> auto { // C++14
    r.B.PRFTBE  = 1u;
    r.B.LATENCY = 1u;
    return r.R;
  });
  ...
  if (HSEStatus == SET) {
    /* Enable Prefetch Buffer and set Flash Latency */
    //Flash.ACR.R = FLASH_ACR_PRFTBE | FLASH_ACR_LATENCY;
    Flash.ACR.setbits([] (Flash_ACR_s & r) -> uint32_t {
      r.B.PRFTBE  = 1u;
      r.B.LATENCY = 1u;
      return r.R;
    });

    RCC.CFGR.modify([] (RCC_CFGR_s & r) -> uint32_t {
      r.B.HPRE     = 0;
      r.B.PPRE     = 0;
      r.B.PLLSRC   = SET;
      r.B.PLLXTPRE = RESET;
